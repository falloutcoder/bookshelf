...
class BooksSelector extends PureComponent {
  ...
  render() {
    ...
    const style = { cursor: atLeastOneBookSelected ? "pointer" : "not-allowed" };

    if (!isSelectMode) {
      return (
        <div className="select-books" onClick={ onSelectModeToggle }></div>
      );
    } else {
      return (
        <div className="selected-books-action">
            <select className="selected-books-move" value="default"
                    style={ style }
